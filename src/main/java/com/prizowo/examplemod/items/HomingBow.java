...
public class HomingBow extends BowItem {
    public HomingBow(Properties properties) {
        super(properties);
    }
        ...
        if (entityLiving instanceof Player player) {
            ...
            if (!ammo.isEmpty()) {
                ...
                if (!level.isClientSide) {
                    ...
                    homingArrow.shootFromRotation(player, player.getXRot(), player.getYRot(), 
                        0.0F, f * 3.0F, 1.0F);
                    
                    homingArrow.setCritArrow(true);
                    homingArrow.pickup = AbstractArrow.Pickup.DISALLOWED;
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(@NotNull Level level, @NotNull Player player, @NotNull InteractionHand hand) {
        ...
        if (!player.getAbilities().instabuild && !flag) {
            ...
            releaseUsing(itemstack, level, player, 0);
